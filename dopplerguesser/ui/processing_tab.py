    ...
    def _prediction_worker(self):
        ...
        try:
            ...
            if dpg.get_value("use_internal_tles"):
                ...
                tle_file_path = dpg.get_value("processing_tle_file_path")

            satellites = fetch_tles(prediction_t_start, tle_file_path=tle_file_path)
...
def draw_processing_tab():
    with dpg.tab(label="Processing"):
        ...
        with dpg.collapsing_header(label="TLEs", default_open=False):
            dpg.add_checkbox(label="Use internal TLEs", tag='use_internal_tles', default_value=True)
            dpg.add_text("If unchecked, you can load TLEs from a file instead:")
            dpg.add_input_text(tag="processing_tle_file_path", default_value="", width=-1)
